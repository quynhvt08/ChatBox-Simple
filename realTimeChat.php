...
$chats = mysqli_query($connect,"SELECT * FROM messages WHERE (FromUser = '".$fromUser."' AND 
                       ToUser ='".$toUser."') OR (FromUser = '".$toUser."'  AND 
                       ToUser ='".$fromUser."' )")
                        ...
                        while($chat = mysqli_fetch_assoc($chats))
                        {
                            ...
                            <p style= 'background: Aquamarine; word-wrap:break-word; display:inline-block;padding:5px; border-radius:10px;max width:70%'>
                            ".$chat["Message"]."
                            </p>
                            </div>";
                        }
